...
namespace osc {
  ...
  void SampleRenderer::render()
  {
    ...
    // int fmsize = launchParams.n_cameras * launchParams.n_directions;
  ...
  void SampleRenderer::setCameraGroup(const Camera* cameras, const int numCameras, 
                                              vec3f* h_directions, const int numDirections)
  {
    ...
    cudaMalloc(&d_directions, numDirections * sizeof(vec3f));

    // Copy data from host to device
    cudaMemcpy(d_positions, h_positions, numCameras * sizeof(vec3f), cudaMemcpyHostToDevice);
    cudaMemcpy(d_directions, h_directions, numDirections * sizeof(vec3f), cudaMemcpyHostToDevice);
    ...
    launchParams.n_directions = numDirections;

    shadowBuffer.resize(numCameras * numDirections * sizeof(bool));
    launchParams.shadowBuffer = (bool*)shadowBuffer.d_pointer();
  }
  


  void SampleRenderer::downloadShadowBuffer(bool h_shadow_buffer[])
  {
    shadowBuffer.download(h_shadow_buffer,
                          launchParams.n_cameras*launchParams.n_directions);
  }
